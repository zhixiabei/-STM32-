...
```
  DC 12V ──→ LM2596(12V→5V) ──→ AMS1117(5V→3.3V)
                  │                    │
                  ↓                    ↓
              L298N(电机)      STM32F103C8T6
                  │               │    │    │
                  ↓          ┌────┼────┼────┼────┐
              直流电机       │    │    │    │    │
                           OLED ESP  DHT11 按键  LED+蜂鸣器
```

- **主控**: STM32F103C8T6（Cortex-M3, 72MHz, 64KB Flash, 20KB SRAM）
- **显示**: 0.96" OLED SSD1306（I2C，4Pin）
- **通信**: ESP8266-01S（UART1, 115200bps）
...
| PA5 | GPIO 输出 | 蜂鸣器（与 LED5 共用，建议二选一） |
...
> 完整引脚对照表请参阅 [Schematic/schematic.md](Schematic/schematic.md) 或 [原理图文档](Auto-Dealer-Based-On-STM32-main/schematic.md)。
